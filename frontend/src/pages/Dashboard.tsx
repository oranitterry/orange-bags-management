    ...
    <div style={{ minHeight: '100vh', background: '#f0f8f4', direction: 'rtl' }}>
      <div style={{ background: '#1a5c38', color: 'white', padding: '12px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          {user?.role === 'superadmin' && (
            <button onClick={() => navigate('/home')} style={{ background: 'rgba(255,255,255,0.2)', color: 'white', border: 'none', borderRadius: '8px', padding: '6px 16px', cursor: 'pointer' }}>
              🏠 בית
            </button>
          )}
        </div>
      ...
      <div style={{ display: 'flex', direction: 'ltr' }}>
        <Sidebar
          ...
          onOpenRound={() => currentRound && openRound(currentRound._id)}
          onCloseRound={() => currentRound && closeRound(currentRound._id)}
          onAddAddress={() => setShowNewAddress(true)}
          roundStatus={currentRound?.status}
        />

        <div style={{ flex: 1, padding: 24, direction: 'rtl' }}>
